 ...
 */
function ConstellationMiniature({ constellation }: { constellation: ConstellationListItem }) {
    const linesAndPoints = useMemo(() => {
        if (!constellation.lines_data || !constellation.pattern_stars || constellation.pattern_stars.length === 0) {
            return null;
        }
...
export function ConstellationSidebar() {
    const { viewMode, timestamp, setViewMode, fetchVisibleStars, setCameraTarget } = useSkyStore();
    ...
    const handleSelect = async (id: number) => {
        ...
        const { bestLocation, selectedConstellation } = useConstellationStore.getState();
        if (bestLocation) {
            const point = {
            ...
            setViewMode("sky");
            await fetchVisibleStars(bestLocation.latitude, bestLocation.longitude, timestamp);

            const currentStars = useSkyStore.getState().stars;
            if (selectedConstellation?.lines_data && currentStars.length > 0) {
                try {
                    const pairs: [number, number][] = JSON.parse(selectedConstellation.lines_data);
                    let cx = 0, cy = 0, cz = 0, count = 0;
                    const starsByHip = new Map(currentStars.filter(s => s.hip_id !== null).map(s => [s.hip_id!, s]));

                    const hipIds = new Set(pairs.flat());
                    for (const hipId of hipIds) {
                        const star = starsByHip.get(hipId);
                        if (star) {
                            const [sx, sy, sz] = altAzToXYZ(star.altitude, star.azimuth, SKY_RADIUS);
                            cx += sx; cy += sy; cz += sz; count++;
                        }
                    }
                    if (count > 0) {
