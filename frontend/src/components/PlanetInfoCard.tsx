...
export const PlanetInfoCard = () => {
    const { viewMode, setViewMode, selectedPlanet, setSelectedPlanet, timestamp } = useSkyStore();
        ...
        >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "5px" }}>
                    ...
                    <span style={{ fontWeight: "bold" }}>{dynamicData.distanceToEarth.toFixed(3)} UA</span>
                </div>

                {selectedPlanet !== "sun" && (
                    <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "5px" }}>
                        <span style={{ color: "#888" }}>Distance au Soleil</span>
                        <span style={{ fontWeight: "bold" }}>{dynamicData.helioDist.toFixed(3)} UA</span>
                    </div>
                )}

                <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "5px" }}>
                    <span style={{ color: "#888" }}>Rayon équatorial</span>
                    <span style={{ fontWeight: "bold" }}>{metadata.radiusKm.toLocaleString()} km</span>
                </div>
